<script type="text/javascript"> 
    ...
    $item.fileupload({
      ...
      progressall: function (e, data) {
        ...
        $('.progress_bar .bar').css('width', progress + '%');
          ...
          if(multiple=='1'){
            file_response += '<input type="hidden" name="'+ file_name +'['+count+']" value="'+ file.name +'" />';
          } else {
            file_response += '<input type="hidden" name="'+ file_name +'" value="'+ file.name +'" />';
          }
  ...
  $('.file_container').on('click', 'a.delete_temp', function(e){
    e.preventDefault();
    $(this).parent().remove();
    $container.isotope( 'layout' );
    $.ajax({
  ...
  $('.progress_bar').on('click', 'a.cancel_upload_button', function(e){
    e.preventDefault();
    xhr.abort();
    return false;
  });
